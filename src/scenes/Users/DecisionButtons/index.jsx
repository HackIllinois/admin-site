...
export default class DecisionButtons extends React.Component {
    ...
    makeDecisionForSelected(decision, wave) {
        this.props.onDecision(
            this.props.selectedUserIds.map((id, index) =>
                this.waitAndUserAction(getDelay(index), id, (userId) =>
                    makeDecision(userId, decision, wave),
                ),
            ),
        )
    }
    ...
    finalizeSelected(finalized = true) {
        this.props.onDecision(
            this.props.selectedUserIds.map((id, index) =>
                this.waitAndUserAction(getDelay(index), id, (userId) =>
                    finalizeDecision(userId, finalized),
                ),
            ),
        )
    }
    ...
    copyAsCsv() {
        ...
        const selectedRegistrations = registrations.filter((registration) =>
            selectedUserIds.includes(registration.id),
        )

        const csv = selectedRegistrations.reduce((str, registration) => {
            const formattedValues = columnKeys
                .map((key) => registration[key])
                .map((value) => {
    ...
    render() {
        ...
        const selectedUsersPassCondition = (condition) =>
            registrations.every(
                (registration) =>
                    !isSelected(registration) || condition(registration),
            )

        const areSelectedAllFinalized = selectedUsersPassCondition(
            (registration) => registration.finalized,
        )
        const areSelectedAllUnfinalized = selectedUsersPassCondition(
            (registration) => !registration.finalized,
        )
        const areSelectedAllDecided = selectedUsersPassCondition(
            (registration) => registration.decisionStatus !== 'PENDING',
        )
        const anySelected = selectedUserIds.length > 0

        const oneThroughTen = Array(10)
            .fill(0)
            .map((_, i) => i + 1)
        ...
        return (
            <div className="decision-buttons">
                ...
                <StyledSelect
                    ...
                    value={{
                        value: selectedWave,
                        label: `Wave ${selectedWave}`,
                    }}
                    menuPortalTarget={document.body}
                    options={oneThroughTen.map((num) => ({
                        value: num,
                        label: `Wave ${num}`,
                    }))}
                    onChange={(selected) =>
                        this.setState({ selectedWave: selected.value })
                    }
                />
                ...
                <button
                    ...
                    onClick={() =>
                        this.makeDecisionForSelected('ACCEPTED', selectedWave)
                    }
                ...
                <button
                    ...
                    onClick={() =>
                        this.makeDecisionForSelected('WAITLISTED', 0)
                    }
                    disabled={!(anySelected && areSelectedAllUnfinalized)}
                >
                    Waitlist
                ...
                <button
                    ...
                    disabled={
                        !(
                            anySelected &&
                            areSelectedAllUnfinalized &&
                            areSelectedAllDecided
                        )
                    }
                >
                    Finalize

...
const getFailedResponseData = async (res) =>
    res.json ? await res.json() : null
...
const Blobstore = () => {
        ...
        if (!blobId) {
            ...
            getBlob(blobId)
                ...
                .catch(async (res) => {
                    ...
                    alert(
                        await createAPIErrorMessage('Failed to get blob', res),
                    )
                })
                .finally(() => {
                    setTimeout(
                        () => setGetButtonText(defaultGetButtonText),
                        resetTextTimeout,
                    )
                })
        }
    ...
    const setBlobData = async () => {
        if (!blobId) {
            ...
            const doesBlobExist = await getBlob(blobId)
                .then(() => true)
                .catch(() => false)
            try {
                if (doesBlobExist) {
                ...
                setTimeout(
                    () => setUpdateButtonText(defaultUpdateButtonText),
                    resetTextTimeout,
                )
            }
        }
    ...
    return (
        <div className="blobstore-page">
            <div className="blobstore-buttons">
                ...
                />
                <button
                    className="get-blob-button"
                    onClick={getBlobData}
                    disabled={getButtonText !== defaultGetButtonText}
                >
                    {getButtonText}
                </button>
                <button
                    className="update-blob-button"
                    onClick={setBlobData}
                    disabled={updateButtonText !== defaultUpdateButtonText}
                >
                    {updateButtonText}
                </button>
            </div>
            ...
            <JSONInput
                ...
                locale={locale}
                height={`${window.innerHeight * 0.8 - 50}px`}
                style={{
                    outerBox: { width: 'auto' },

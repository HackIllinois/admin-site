...
export default class Statistics extends React.Component {
    ...
    reload() {
        ...
        Promise.all([
            ...
            .then(([stats, events, ...userData]) => [
                stats,
                events,
                addOtherData(...userData),
            ])
            .then(([stats, events, registrations]) => {
                this.setState({ stats, registrations, isLoading: false })

                const eventsToCount = events.filter((event) =>
                    countEventTypes.includes(event.eventType),
                )
                ...
                Promise.all(trackerRequests).then((trackers) => {
                    const eventCounts = trackers.map((tracker) => ({
                        name: eventsToCount.find(
                            (event) => event.id === tracker.eventId,
                        ).name,
            ...
            <div className="statistics-page">
                ...
                <div className="options">
                    <StyledSelect
                        ...
                        onChange={(selected) =>
                            this.handleGroupSelect(selected)
                        }
                    />

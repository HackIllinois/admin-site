...
export default function LocationCheckbox({ field, form }) {
    ...
    const addLocation = (location, otherInfo = {}) => {
        ...
        const { details = getDetails(location) } = otherInfo
        ...
        form.setFieldValue(
            ...
            selectedLocations
                .filter(
                    (selected) =>
                        !selected.description.includes(location.description),
                )
                .concat(newLocation),
        )
    }
    ...
    const findSelectedLocation = (name) =>
        selectedLocations.find(({ description }) =>
    ...
    const getDetails = (location) => {
        ...
        const selectedDescription = findSelectedLocation(
            ...
            {locations.map((location) => (
                <div className="location-option" key={location.description}>
                    ...
                    <input
                        ...
                        onChange={(e) =>
                            addLocation(location, { details: e.target.value })
                        }
                    />
                </div>
            ))}

...
export class AuthService {
    ...
    /**
     * Gets OAuth URL for authentication
     * **Required role: null**
     *
     * Returns the OAuth URL that the client should redirect to for authentication. Attendees authenticate through GitHub, and staff authenticate through Google. The redirect parameter must be a valid origin with an optional relative path. After successful authentication, the user will be redirected to the provided URL with JWT and refresh tokens set as HTTP-only cookies.
     */
    public static getAuthByProvider<ThrowOnError extends boolean = false>(options: Options<GetAuthByProviderData, ThrowOnError>) {
        return (options.client ?? _heyApiClient).get<GetAuthByProviderResponse, GetAuthByProviderError, ThrowOnError>({
            security: [
                {
                    scheme: 'bearer',
                    type: 'http'
